...
public class Exam04 extends HttpServlet {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            out.printf(savePath + "/" + filename);
            photo.write(new File(savePath + "/" + filename));
...
//        String extName = originFilename.substring(lastIndex+1);
        ...
        String[] arrays = originFilename.split(".");
        
        if(arrays.length > 2)
            return String.format("%d-%d.%s", System.currentTimeMillis(), ++count, arrays[arrays.length-1] );
        else
            return String.format("%d-%d%s", System.currentTimeMillis(), ++count, originFilename );
    }
}
