...
SQL문에서 DB객체(테이블, 뷰, 함수, 트리거 등)를 생성, 변경, 삭제을 담당하는 언어.
...
데이터 입력 테스트:
> insert into test01(no, name) values(1, 'aaa');
> insert into test01(no, name) values(null, 'bbb');
> insert into test01(no, name) values(3, null);
> insert into test01(no, name) values(null, null);
> select * from test01;
...
데이터 입력 테스트:
> insert into test01(no, name) values(1, 'aaa');
> insert into test01(no, name) values(null, 'bbb'); /* 실행 오류 */
> insert into test01(no, name) values(3, null);
...
> insert into test01(no, name, age) values(1, 'aaa', 30);

값을 입력하지 않는 컬럼은 이름과 값 지정을 생략한다.
> insert into test01(name, age) values('aaa', 30); /* 오류! no는 not null*/
> insert into test01(no, age) values(3, 30);
> insert into test01(no, name) values(4, 'ddd');
> insert into test01(no) values(5);
...
> insert into test01(c1) values(100);
> insert into test01(c1) values(3.14); /* 소수점 짤림 */
> insert into test01(c2) values(100);
> insert into test01(c2) values(3.14);
> insert into test01(c2) values(3.14159); 
> insert into test01(c3) values(100);
> insert into test01(c3) values(123456789); /* 입력 오류. 4자리 초과 */
> insert into test01(c3) values(12345); /* 입력 오류. 4자리 초과 */
> insert into test01(c3) values(1234);
> insert into test01(c3) values(3.14);
> insert into test01(c3) values(3.14159); /* 2자리를 초과한 값은 반올림. */
> insert into test01(c3) values(3.14659); /* 2자리를 초과한 값은 반올림. */
> insert into test01(c4) values(1234567890); 
> insert into test01(c4) values(12.34567890); /* 소수점은 반올림 처리됨 */
> insert into test01(c4) values(12345678.90); /* 소수점은 반올림 처리됨 */
...
입력 테스트:
> insert into test01(c1) values('');
> insert into test01(c1) values('abcde');
> insert into test01(c1) values('abcdefghi'); /* 입력 크기 초과 오류! */
> insert into test01(c1) values('가나다라마'); /* 한글 영어 상관없이 5자 */
> insert into test01(c2) values('');
> insert into test01(c2) values('abcde');
> insert into test01(c2) values('abcdefghi'); /* 입력 크기 초과 오류! */

고정 크기와 가변 크기 비교
- insert into test01(c1) values('abc');
- insert into test01(c2) values('abc');
> select * from test01  where c1='abc  ';
/* DBMS 중에서는 고정 크기인 컬럼의 값을 비교할 때, 빈자리까지 검사하는 경우도 있음. */
/* 즉, c1='abc'로는 데이터를 못찾고 'abc  '여야 데이터를 찾는 경우가 있다. */
/* 하지만 mySql은 고정크기여도 빈자리를 무시하고 값을 찾는다. */
...
입력 테스터:
> insert into test01(c1) values('2017-11-21');
> insert into test01(c2) values('16:12:35');
> insert into test01(c3) values('2017-11-21 16:13:33');
> insert into test01(c1) values('2017-11-21 16:13:33'); /* 날짜 정보만 저장*/
> insert into test01(c2) values('2017-11-21 16:13:33'); /* 시간 정보만 저장*/
...
> insert into test01(c1) values('Y'); /* yes */
> insert into test01(c1) values('N'); /* no */
> insert into test01(c1) values('T'); /* true */
> insert into test01(c1) values('F'); /* false */
> insert into test01(c1) values('1'); /* true */
> insert into test01(c1) values('0'); /* false */
> insert into test01(c2) values(1); /* true */
> insert into test01(c2) values(0); /* false */
...
- 그래서 실무에서는 이런 경우 '학번'과 같이 별도로 임의의 값을 저장하는 컬럼을 만들어 PK로 사용한다.
...
- 그래서 PK를 대신해서 사용할 수 있는 key라고 해서 "대체키"라고 부른다.
...
- 단 primarykey에 대해서만 자동 증가 옵션을 붙일 수 있다.
```
alter table test1
  add constraint primary key (no),
